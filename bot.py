    ...
    while True:
        try:
            ...
            for f in upcoming:
                ...
                # Trigger lineup fetch exactly around 60 minutes before kickoff
                if 59 <= diff_mins <= 61:  
                    logging.info(f"Auto-checking match: {f['team_h_name']} vs {f['team_a_name']}")
                    
                    sofa_events = get_today_sofascore_matches()
                    target_event = next(
                        (e for e in sofa_events if e.get('homeTeam', {}).get('name') == f['team_h_name'] 
...
async def start(update: Update, context: CallbackContext):
    ...
    welcome_msg = (
        ...
        "This bot monitors lineups and alerts you when players with high ownership are benched or when tactical shifts occur.\n\n"
        "Commands available:\n"
        "/start - Show this message and register you for alerts\n"
